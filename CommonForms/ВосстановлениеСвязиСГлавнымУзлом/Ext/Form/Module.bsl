...
Процедура ОтключитьНаСервере()
	...
	Попытка
		...
		РаботаВАвтономномРежиме			= Константы.РаботаВАвтономномРежиме.СоздатьМенеджерЗначения();
		РаботаВАвтономномРежиме.Прочитать();
		Если РаботаВАвтономномРежиме.Значение Тогда
			РаботаВАвтономномРежиме.Значение = Ложь;
			ОбновлениеИБСервер.ЗаписатьДанные(РаботаВАвтономномРежиме);
		КонецЕсли;
...
Процедура СоздатьНедостающиеПредопределенныеДанные()
	...
	Для Каждого Коллекция Из КоллекцииМетаданных Цикл
		...
		Для Каждого ОбъектМетаданных Из Коллекция Цикл
			...
			СохранитьСуществующиеПредопределенныеОбъектыПередСозданиемНедостающих(ОбъектМетаданных, ПолноеИмя, ТаблицаИмен, Имена, Запрос, ОписанияСохраненных, ЭтоПланСчетов);
		КонецЦикла;
	КонецЦикла;
	...
	Для Каждого ОписаниеСохраненных Из ОписанияСохраненных Цикл
		...
		Для Каждого ОписаниеСохраненного Из ОписаниеСохраненных.ТаблицаИмен Цикл
			...
			Если Строка <> Неопределено Тогда
				...
				КонецЕсли;
				ОбновлениеИБСервер.УдалитьДанные(НовыйОбъект);
				Строка.Имя = "";
			КонецЕсли;
			ОбновлениеИБСервер.ЗаписатьДанные(ОписаниеСохраненного.Объект);
		КонецЦикла;
		Для Каждого Строка Из ТаблицаИмен Цикл
			...
			Если СтрокаРодителя <> Неопределено Тогда
				...
				ОбновлениеИБСервер.ЗаписатьДанные(НовыйОбъект);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
КонецПроцедуры
		...
		Если Счет.ВидыСубконто.Количество() > Индекс Тогда
			Если Счет.ВидыСубконто[Индекс].ВидСубконто <> Строка.ВидСубконто Тогда
				ЗаписьЖурналаРегистрации("Обмен данными.Отключение связи с главным узлом",
					...
					Счет,
					СтрШаблон(
						"У счета ""%1"" субконто №%2 ""%3"" не совпадает с предопределенным субконто ""%4"".",
						Строка(Счет),
						Индекс + 1,
...
&НаСервереБезКонтекста
Процедура СохранитьСуществующиеПредопределенныеОбъектыПередСозданиемНедостающих(ОбъектМетаданных, ПолноеИмя, ТаблицаИмен, Имена, Запрос, ОписанияСохраненных, ЭтоПланСчетов)
		...
		Для каждого Строка Из ТаблицаИмен Цикл
			Если Строка.ОбъектСуществует Тогда
				...
				Если ЭтоПланСчетов Тогда
					...
					КонецЦикла;
				КонецЕсли;
				ОбновлениеИБСервер.ЗаписатьДанные(Объект);
				Если ЭтоПланСчетов Тогда
	...
	Менеджер = БазоваяПодсистемаСервер.МенеджерОбъектаПоПолномуИмени(ПолноеИмя);
