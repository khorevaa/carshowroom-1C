...
Процедура Подключаемый_ПроверитьВыполнениеЗадания()
	...
	ИначеЕсли Статус = "Ошибка" Тогда
		...
		Если ВозвращатьРезультатВОбработкуВыбора() Тогда
			...
			ПараметрыПредупреждения.Вставить("Заголовок", "Не удалось выполнить запрошенное действие");
			БазоваяПодсистемаКлиент.ВывестиПредупреждение(ВладелецФормы, ПараметрыПредупреждения);
		КонецЕсли;
...
Функция ПроверитьЗаданиеВыполнено(ФормаЗакрывается)
	Задание = БазоваяПодсистемаСервер.ОперацияВыполнена(ИдентификаторЗадания, Ложь, Параметры.ВыводитьПрогрессВыполнения, Параметры.ВыводитьСообщения);
...
Функция ВозвращатьРезультатВОбработкуВыбора()
	Возврат ОписаниеОповещенияОЗакрытии = Неопределено И Параметры.ПолучатьРезультат И ТипЗнч(ВладелецФормы) = Тип("УправляемаяФорма");
КонецФункции
