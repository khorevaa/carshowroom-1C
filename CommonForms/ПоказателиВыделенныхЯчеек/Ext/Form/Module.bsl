...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Расчет = БазоваяПодсистемаКлиентСервер.СвойствоСтруктуры(Параметры, "Расчет");
	Если ТипЗнч(Расчет) <> Тип("Структура") Тогда
		Расчет = БазоваяПодсистемаКлиентСервер.РасчетЯчеек(Параметры.ТабличныйДокумент, Параметры.ВыделенныеОбласти);
	КонецЕсли;
