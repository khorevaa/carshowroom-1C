...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ФормаПараметры = Новый Структура("КлючНазначенияИспользования, КлючПользовательскихНастроек, Расшифровка, СформироватьПриОткрытии, ТолькоПросмотр, ФиксированныеНастройки, Раздел, Подсистема, ПодсистемаПредставление");
	...
	ПолныйПуть = БазоваяПодсистемаКлиентСервер.СвойствоСтруктуры(Параметры, "ПолныйПутьТекущегоУзлаКД");
	Если ЗначениеЗаполнено(ПолныйПуть) Тогда
		НастройкиКД = БазоваяПодсистемаКлиентСервер.СвойствоСтруктуры(Параметры, "Вариант");
		Если НастройкиКД = Неопределено Тогда
			НастройкиКД = Отчет.КомпоновщикНастроек.Настройки;
		КонецЕсли;
		КорневойУзел = ВариантыОтчетовКлиентСервер.НайтиЭлементПоПолномуПути(НастройкиКД, ПолныйПуть);
	...
	Если ТипЗнч(ИдентификаторТекущегоУзлаКД) <> Тип("ИдентификаторКомпоновкиДанных") Тогда
		ИдентификаторТекущегоУзлаКД = БазоваяПодсистемаКлиентСервер.СвойствоСтруктуры(Параметры, "ТекущаяСтрока");
		Если ТипЗнч(ИдентификаторТекущегоУзлаКД) <> Тип("ИдентификаторКомпоновкиДанных") Тогда
			ИдентификаторТекущегоУзлаКД = БазоваяПодсистемаКлиентСервер.СвойствоСтруктуры(Параметры, "ИдентификаторТекущегоУзлаКД");
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПередЗагрузкойВариантаНаСервере(НовыеНастройкиКД)
	...
	Если ТипЗнч(ФормаПараметры.Отбор) = Тип("Структура") Тогда
		ВариантыОтчетовСервер.УстановитьФиксированныеОтборы(ФормаПараметры.Отбор, НовыеНастройкиКД, НастройкиОтчета);
	КонецЕсли;
КонецПроцедуры
