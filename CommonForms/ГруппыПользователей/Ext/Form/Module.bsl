...
Процедура ЗаписатьИЗакрытьНачало(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если УведомлениеПользователя.ЕстьОшибки = Ложь Тогда
		Если УведомлениеПользователя.Сообщение <> Неопределено Тогда
			ПоказатьОповещениеПользователя("Перемещение пользователей", , УведомлениеПользователя.Сообщение, БиблиотекаКартинок.Информация);
		КонецЕсли;
	Иначе
			...
			Если Группа.Родитель = ПустаяГруппа Тогда
				...
				Если СписокПользователей.КоличествоПользователей = 1 Тогда
					...
					Если ТипПользователя = "ВнешнийПользователь" Тогда
						...
						Назначение			= БазоваяПодсистемаСервер.ЗначениеРеквизитаОбъекта(Группа.Ссылка, "Назначение").Выгрузить();
...
Процедура ПолучитьГруппыВнешнихПользователей(ГруппыПользователей)
	...
	Запрос.Текст	= "ВЫБРАТЬ
	            	  ...
	            	  |	ГруппыВнешнихПользователей.Родитель КАК Родитель,
	            	  |	ГруппыВнешнихПользователей.ВсеОбъектыАвторизации КАК ВсеОбъектыАвторизации
	            	  |ИЗ
	...
	Для Каждого СтрокаДерева Из ДеревоГруппИсточник Цикл
		Если СтрокаДерева.Пометка = 1 И Не СтрокаДерева.ГруппаНеИзменяется Тогда
			Для Каждого ПользовательСсылка Из МассивПользователей Цикл
				...
				Иначе
					ТипПользователя		= "ВнешнийПользователь";
					ПеремещениеВозможно	= ПользователиСервер.ВозможноПеремещениеПользователя(СтрокаДерева.Группа, ПользовательСсылка);
				...
				Если Добавлять Тогда
					ПользователиСервер.ДобавитьПользователяВГруппу(СтрокаДерева.Группа, ПользовательСсылка, ТипПользователя);
		...
		ИначеЕсли СтрокаДерева.Пометка = 0 И Не СтрокаДерева.ГруппаНеИзменяется Тогда
			Для Каждого ПользовательСсылка Из МассивПользователей Цикл
				...
				Если Удалять Тогда
					ПользователиСервер.УдалитьПользователяИзГруппы(СтрокаДерева.Группа, ПользовательСсылка, ТипПользователя);
...
Процедура СформироватьТекстСообщения(МассивПеремещенныхПользователей, УведомлениеПользователя, НеПеремещенныеПользователи)
	...
	Если КоличествоНеПеремещенныхПользователей > 0 Тогда
		Если КоличествоНеПеремещенныхПользователей = 1 Тогда
			...
			СообщениеПользователю = СтрШаблон("Пользователя ""%1"" не удалось включить в выбранные группы,
				...
				Предмет);
		Иначе
			Предмет					= СтрокаСЧислом("%1 пользователю;;%1 пользователям;%1 пользователям;%1 пользователям", КоличествоНеПеремещенныхПользователей, ВидЧисловогоЗначения.Количественное);
	...
	ИначеЕсли КоличествоПользователей = 1 Тогда
		НаименованиеПользователя = БазоваяПодсистемаСервер.ЗначениеРеквизитаОбъекта(МассивПеремещенныхПользователей[0], "Наименование");
...
Процедура ЗаписатьИЗакрытьОбработкаВопроса(Ответ, Отчет) Экспорт
	Если Ответ = "Ок" Тогда
		Возврат;
	Иначе
		Отчет.Показать("Пользователи, не включенные в группы");
		Возврат;
	КонецЕсли;
