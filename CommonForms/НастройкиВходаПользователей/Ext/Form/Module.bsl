...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПоказатьНастройкиВнешнихПользователей Тогда
		БазоваяПодсистемаСервер.УстановитьКлючНазначенияФормы(ЭтотОбъект, "ВнешниеПользователи");
		АвтоЗаголовок	= Ложь;
		Заголовок		= "Настройки входа внешних пользователей";
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, ПользователиСервер.НастройкиВхода().ВнешниеПользователи);
	Иначе
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, ПользователиСервер.НастройкиВхода().Пользователи);
	КонецЕсли;
	...
	Если ПолучитьПроверкуСложностиПаролейПользователей() Тогда
		ТекстВопросаОбОчисткеНастроекКонфигуратора = СтрШаблон("Установлена минимальная длина паролей %1 и требование сложности паролей в конфигураторе
	...
	ИначеЕсли ПолучитьМинимальнуюДлинуПаролейПользователей() > 0 Тогда
		ТекстВопросаОбОчисткеНастроекКонфигуратора = СтрШаблон("Установлена минимальная длина паролей %1 в конфигураторе
			           |в меню ""Администрирование"" в пункте ""Параметры информационной базы ..."".
			           |
...
Процедура ПриОткрытии(Отказ)
	Если ЗначениеЗаполнено(ТекстВопросаОбОчисткеНастроекКонфигуратора) Тогда
		...
		Кнопки.Добавить("Очистить", "Очистить");
		Кнопки.Добавить("Отмена",   "Отмена");
		ПоказатьВопрос(Новый ОписаниеОповещения("ПриОткрытииПослеОтветаНаВопрос", ЭтотОбъект), ТекстВопросаОбОчисткеНастроекКонфигуратора, Кнопки);
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаписатьНаСервере()
	...
	Попытка
		...
		НастройкиВхода = ПользователиСервер.НастройкиВхода();
		...
		ИначеЕсли Не ЗначениеЗаполнено(Настройки.ПросрочкаРаботыВПрограммеДатаВключения) Или Не ЗначениеЗаполнено(Настройки.ПросрочкаРаботыВПрограммеДоЗапрещенияВхода) Тогда
