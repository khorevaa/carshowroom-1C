...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПолныеПраваНаВарианты = ПравоДоступа("Изменение", Метаданные.Справочники.ВариантыОтчетов);
	ПравоНаЭтотВариант = ПолныеПраваНаВарианты Или Объект.Автор = ПользователиКлиентСервер.АвторизованныйПользователь();
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если Источник <> ЭтотОбъект И (ИмяСобытия = "СтандартныеПодсистемы.ВариантыОтчетов.ИзменениеВарианта" Или ИмяСобытия = "Запись_НаборКонстант") Тогда
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ТипЗнч(СвойстваПредопределенного) = Тип("ФиксированнаяСтруктура") Тогда
		ТекущийОбъект.ВидимостьПоУмолчаниюПереопределена = Объект.ВидимостьПоУмолчанию <> СвойстваПредопределенного.ВидимостьПоУмолчанию;
	...
	КонецЕсли;
	ВариантыОтчетовСервер.ДеревоПодсистемЗаписать(ТекущийОбъект, ИзмененныеРазделы);
КонецПроцедуры
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Оповестить("СтандартныеПодсистемы.ВариантыОтчетов.ИзменениеВарианта", ПараметрОповещения, ЭтотОбъект);
	БазоваяПодсистемаКлиент.РазвернутьУзлыДерева(ЭтотОбъект, "ДеревоПодсистем", "*", Истина);
КонецПроцедуры
...
Процедура ОписаниеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ВариантыОтчетовКлиент.РедактироватьМногострочныйТекст(ЭтотОбъект, Элемент.ТекстРедактирования, Объект, "Описание", "Описание");
КонецПроцедуры
...
Функция ПерезаполнитьДерево(Прочитать)
	ВыделенныеСтроки = ВариантыОтчетовСервер.ЗапомнитьВыделенныеСтроки(ЭтотОбъект, "ДеревоПодсистем", "Ссылка");
	Если Прочитать Тогда
		ЭтотОбъект.Прочитать();
	КонецЕсли;
	ДеревоПриемник = ВариантыОтчетовСервер.ДеревоПодсистемСформировать(ЭтотОбъект, Объект);
	ЗначениеВРеквизитФормы(ДеревоПриемник, "ДеревоПодсистем");
	ВариантыОтчетовСервер.ВосстановитьВыделенныеСтроки(ЭтотОбъект, "ДеревоПодсистем", ВыделенныеСтроки);
...
Процедура ПрочитатьСвойстваПредопределенного(ПервоеЧтение)
	Если ПервоеЧтение Тогда
		Если Не Объект.Пользовательский
			И (Объект.ТипОтчета = Перечисления.ТипыОтчетов.Внутренний Или Объект.ТипОтчета = Перечисления.ТипыОтчетов.Расширение)
			И ЗначениеЗаполнено(Объект.ПредопределенныйВариант) Тогда // Чтение настроек предопределенного.
			Сведения					= БазоваяПодсистемаСервер.ЗначенияРеквизитовОбъекта(Объект.ПредопределенныйВариант, "ВидимостьПоУмолчанию, Описание");
