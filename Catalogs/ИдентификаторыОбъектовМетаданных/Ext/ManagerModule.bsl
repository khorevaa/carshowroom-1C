...
Процедура ВыполнитьОбновлениеДанных(ЕстьИзменения, ЕстьУдаленные, ТолькоПроверка, ЕстьКритичныеИзменения = Ложь, СписокКритичныхИзменений = "", ОбъектыРасширений = Ложь) Экспорт
	Если ОбъектыРасширений И ЗначениеЗаполнено(ПараметрыСеанса.ПодключенныеРасширения) И Не Справочники.ВерсииРасширений.УстановленныеРасширенияДоступны() Тогда
		ТекстОшибки = "Невозможно обновить идентификаторы объектов расширений
			           |в сеансе неразделенного пользователя, так как
			           |расширения для разделенных пользователей не подключены.";
		ЗаголовокОшибки = СтрШаблон("Ошибка при работе со справочником ""%1"".", НазваниеСправочника(Истина));
	...
	Если Не ОбъектыРасширений Или Не БазоваяПодсистемаСервер.ЭтоПодчиненныйУзелРИБ() Тогда
	...
	Если Не ОбъектыРасширений Тогда
		РегистрыСведений.ПараметрыРаботыПрограммы.УстановитьПараметрРаботыПрограммы("СтандартныеПодсистемы.ИдентификаторыОбъектовМетаданных", Истина);
	КонецЕсли;
КонецПроцедуры
...
Функция ИдентификаторОбъектаМетаданных(ОписаниеОбъектаМетаданных) Экспорт
	...
	Если ТипОписанияОбъектаМетаданных = Тип("Тип") Тогда
		...
		Если ОбъектМетаданных = Неопределено Тогда
			ВызватьИсключение СтрШаблон("Ошибка при выполнении функции Справочники.ИдентификаторыОбъектовМетаданных.ИдентификаторОбъектовМетаданных().
				           |
	...
	Возврат БазоваяПодсистемаСерверПовтИсп.ИдентификаторОбъектаМетаданныхПоПолномуИмени(ПолноеИмяОбъектаМетаданных);
КонецФункции
...
Функция ИдентификаторыОбъектовМетаданныхСПопыткойПовтора(ПолныеИменаОбъектовМетаданных, ОдинЭлемент = Ложь) Экспорт
	БазоваяПодсистемаСерверПовтИсп.ИдентификаторыОбъектовМетаданныхПроверкаИспользования(Истина, Ложь);

	Попытка
		Идентификаторы = ИдентификаторыОбъектовМетаданныхБезПопыткиПовтора(ПолныеИменаОбъектовМетаданных, ОдинЭлемент);
	Исключение
		Идентификаторы = Неопределено;
	КонецПопытки;

	Если Идентификаторы = Неопределено Тогда
		ВыполнитьОбновлениеДанных(Ложь, Ложь, Ложь);
		Идентификаторы = ИдентификаторыОбъектовМетаданныхБезПопыткиПовтора(ПолныеИменаОбъектовМетаданных, ОдинЭлемент);
	КонецЕсли;
...
Функция ИдентификаторыОбъектовМетаданныхБезПопыткиПовтора(ПолныеИменаОбъектовМетаданных, ОдинЭлемент)
	...
	Если КоличествоОшибок > 0 Тогда
		Если ОдинЭлемент Тогда
			ЗаголовокОшибки = "Ошибка при выполнении функции Справочники.ИдентификаторыОбъектовМетаданных.ИдентификаторОбъектовМетаданных().";
		ИначеЕсли КоличествоОшибок = 1 Тогда
			ЗаголовокОшибки = "Ошибка при выполнении функции Справочники.ИдентификаторыОбъектовМетаданных.ИдентификаторыОбъектовМетаданных().";
		Иначе
			ЗаголовокОшибки = "Ошибки при выполнении функции Справочники.ИдентификаторыОбъектовМетаданных.ИдентификаторыОбъектовМетаданных().";
		КонецЕсли;
		...
		Для Каждого ОписаниеОшибки Из Ошибки Цикл
			...
			Если НомерОшибки = 3 И КоличествоОшибок > 5 Тогда
				ПредставлениеКоличества	= СтрокаСЧислом("%1 ошибка;;%1 ошибки;%1 ошибок;;%1 ошибки",КоличествоОшибок - НомерОшибки, ВидЧисловогоЗначения.Количественное, "L=ru");
				ОписаниеОшибки			= СтрШаблон("... И еще %1", ПредставлениеКоличества);
...
Процедура ПроверкаИспользования(ОбъектыРасширений = Ложь) Экспорт
	Если БазоваяПодсистемаСерверПовтИсп.ОтключитьИдентификаторыОбъектовМетаданных() Тогда
	...
	Если ПланыОбмена.ГлавныйУзел() = Неопределено
	   И ЗначениеЗаполнено(БазоваяПодсистемаСервер.МенеджерОбъектаПоПолномуИмени("Константа.ГлавныйУзел").Получить()) Тогда
...
Процедура ЗаменитьДублиПодчиненногоУзлаНайденныеПриЗагрузке(ТолькоПроверка, ЕстьИзменения)
	...
	Запрос.Текст	= "ВЫБРАТЬ
	            	  |	Идентификаторы.Ссылка КАК Ссылка,
	            	  |	Идентификаторы.НоваяСсылка КАК НоваяСсылка
	            	  |ИЗ
	            	  |	Справочник.ИдентификаторыОбъектовМетаданных КАК Идентификаторы
	            	  |ГДЕ
...
Процедура ОбновитьДанные(ЕстьИзменения, ЕстьУдаленные, ТолькоПроверка, ЕстьКритичныеИзменения, СписокКритичныхИзменений, ОбъектыРасширений)
	...
	СправочникМенеджер			= БазоваяПодсистемаСервер.МенеджерОбъектаПоПолномуИмени(ИмяСправочника(ОбъектыРасширений));
	...
	Попытка
		...
		Если НЕ ОбъектыРасширений
		   И НЕ БазоваяПодсистемаСервер.ЭтоПодчиненныйУзелРИБ() Тогда
		...
		Если Не ТолькоПроверка И Не ОбъектыРасширений И ЗначениеЗаполнено(СписокКритичныхИзменений) И БазоваяПодсистемаСервер.ЭтоПодчиненныйУзелРИБ() Тогда
			...
			ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + "Критичные изменения могут быть выполнены только
	...
	Попытка
		Для Каждого СтрокаТаблицы Из ТаблицаСсылок Цикл
				...
				Если ОбрабатываемаяСсылка <> Неопределено И Параметры.Объект <> Неопределено Тогда
					Попытка
						...
						Если ОтключатьКонтрольЗаписи Тогда
							...
							ОбновлениеИБСервер.ЗаписатьДанные(Параметры.Объект, Ложь);
						Иначе
					...
					Если НадоЗаписывать Тогда
						...
						Попытка
							Если ОтключатьКонтрольЗаписи Тогда
								...
								ОбновлениеИБСервер.ЗаписатьДанные(НаборЗаписей, Ложь);
							Иначе
							...
							Если НадоЗаписывать Тогда
								Попытка
									Если ОтключатьКонтрольЗаписи Тогда
										...
										ОбновлениеИБСервер.ЗаписатьДанные(НаборЗаписи, Ложь);
									Иначе
			...
			ИначеЕсли Метаданные.Константы.Содержит(СтрокаТаблицы.Метаданные) Тогда
				БазоваяПодсистемаСервер.МенеджерОбъектаПоПолномуИмени(СтрокаТаблицы.Метаданные.ПолноеИмя()).Установить(ПравильныйЭлемент);
			ИначеЕсли Метаданные.РегистрыСведений.Содержит(СтрокаТаблицы.Метаданные) Тогда
				СтруктураИзмерений	= Новый Структура;
				МенеджерРегистра	= БазоваяПодсистемаСервер.МенеджерОбъектаПоПолномуИмени(СтрокаТаблицы.Метаданные.ПолноеИмя());
				...
				КонецПопытки;
			Иначе
				СообщитьОбОшибке("Значения не заменяются в данных типа" + ": " + Строка(СтрокаТаблицы.Метаданные), ОбъектыРасширений);
			КонецЕсли;
		КонецЦикла;
...
Функция СвойстваОбъектовМетаданных(ОбъектыРасширений, СвойстваКоллекций = Неопределено)
	...
	Если СвойстваКоллекций = Неопределено Тогда
		СвойстваКоллекций = БазоваяПодсистемаСерверПовтИсп.СвойстваКоллекцийОбъектовМетаданных(ОбъектыРасширений);
	КонецЕсли;
	...
	Для каждого СвойстваКоллекции Из СвойстваКоллекций Цикл
		ДобавитьСвойстваОбъектовМетаданных(Метаданные[СвойстваКоллекции.Имя], СвойстваКоллекции, СвойстваОбъектовМетаданных, ОбъектыРасширений);
	КонецЦикла;
...
Функция ВыгрузкаВсехИдентификаторов(ОбъектыРасширений = Ложь)
	...
	Запрос.Текст	=  "ВЫБРАТЬ
	            	   ...
	            	   |	Идентификаторы.ПорядокКоллекции КАК ПорядокКоллекции,
	            	   |	Идентификаторы.Имя КАК Имя,
	            	   |	Идентификаторы.Синоним КАК Синоним,
	            	   |	Идентификаторы.ПолноеИмя КАК ПолноеИмя,
	            	   |	Идентификаторы.ПолныйСиноним КАК ПолныйСиноним,
	            	   |	Идентификаторы.БезДанных КАК БезДанных,
	            	   |	Идентификаторы.ЗначениеПустойСсылки КАК ЗначениеПустойСсылки,
	            	   ...
	            	   |	Справочник.ИдентификаторыОбъектовМетаданных КАК Идентификаторы";
	Если ОбъектыРасширений Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Справочник.ИдентификаторыОбъектовМетаданных", "Справочник.ИдентификаторыОбъектовРасширений");
	КонецЕсли;
...
Процедура ПереименоватьПолныеИмена(Выгрузка, СписокПереименованийОбъектовМетаданных = "", ЕстьКритичныеИзменения = Ложь)
	ТаблицаПереименования	= БазоваяПодсистемаСерверПовтИсп.ТаблицаПереименованияДляТекущейВерсии();
...
Функция ТаблицаПереименованияДляТекущейВерсии() Экспорт
	...
	КоллекцииБезКлючаОбъектаМетаданных = БазоваяПодсистемаСерверПовтИсп.СвойстваКоллекцийОбъектовМетаданных().НайтиСтроки(Отбор);
...
Процедура ДобавитьСвойстваОбъектовМетаданных(Знач КоллекцияОбъектовМетаданных, Знач СвойстваКоллекции, Знач СвойстваОбъектовМетаданных, Знач ОбъектыРасширений, Знач ПолноеИмяРодителя = "", Знач ПолныйСинонимРодителя = "")
	Для каждого ОбъектМетаданных Из КоллекцияОбъектовМетаданных Цикл
		...
		Если НЕ СвойстваКоллекции.БезДанных И СтрНайти(СвойстваКоллекции.ИмяВЕдЧисле, "Регистр") = 0 И СтрНайти(СвойстваКоллекции.ИмяВЕдЧисле, "Константа") = 0 Тогда
		...
		Если СвойстваКоллекции.Имя = "Подсистемы" Тогда
			ДобавитьСвойстваОбъектовМетаданных(ОбъектМетаданных.Подсистемы, СвойстваКоллекции, СвойстваОбъектовМетаданных, ОбъектыРасширений, ПолноеИмя, НоваяСтрока.ПолныйСиноним + ". ");
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Функция КлючОбъектаМетаданных(ПолноеИмя)
	...
	ИначеЕсли ВРег(КлассОМ) = ВРег("Константа") Тогда
		Возврат ТипЗнч(БазоваяПодсистемаСервер.МенеджерОбъектаПоПолномуИмени(ПолноеИмя));
...
Процедура ПроверитьОбъектПередЗаписью(Объект, АвтоматическоеОбновление = Ложь)
	...
	Если НЕ АвтоматическоеОбновление Тогда
		Если Объект.ЭтоНовый() Тогда
			...
			ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + "Создание нового идентификатора объекта
				           |возможно только автоматически при обновлении данных справочника.";
		ИначеЕсли ЗапрещеноИзменятьПолноеИмя(Объект) Тогда
			ЗаголовокОшибки = СтрШаблон("Ошибка при работе со справочником ""%1"".", НазваниеСправочника(ОбъектыРасширений));

			ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтрШаблон("При изменении идентификатора объекта указано
	...
	Если Не ОбъектыРасширений И БазоваяПодсистемаСервер.ЭтоПодчиненныйУзелРИБ() Тогда
		Если Объект.ЭтоНовый() И Не ЭтоКоллекция(Объект.ПолучитьСсылкуНового(), ЭтоОбъектРасширений(Объект)) Тогда
			ЗаголовокОшибки = СтрШаблон("Ошибка при работе со справочником ""%1"".", НазваниеСправочника(ОбъектыРасширений));

			ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + "Добавление новых элементов может быть выполнено
		...
		Если Не Объект.ПометкаУдаления И Не ЭтоКоллекция(Объект.Ссылка, ЭтоОбъектРасширений(Объект)) Тогда
			Если ВРег(Объект.ПолноеИмя) <> ВРег(БазоваяПодсистемаСервер.ЗначениеРеквизитаОбъекта(Объект.Ссылка, "ПолноеИмя")) Тогда
				ЗаголовокОшибки = СтрШаблон("Ошибка при работе со справочником ""%1"".", НазваниеСправочника(ОбъектыРасширений));

				ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + "Изменение реквизита ""Полное имя"" может быть выполнено
					           |только в главном узле распределенной информационной базы.";
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьСписокНовыхПодсистем(НовыеПодсистемы)
	Сведения		= ОбновлениеИБСервер.СведенияОбОбновленииИнформационнойБазы();
	ЕстьИзменения	= Ложь;
	...
	ОбновлениеИБСервер.ЗаписатьСведенияОбОбновленииИнформационнойБазы(Сведения);
КонецПроцедуры
...
Функция ПолноеИмяИспользуется(Объект, ОбъектыРасширения)
	...
	Если ОбъектыРасширения Тогда
		ТекстЗапроса = СтрЗаменить(Запрос.Текст, "Справочник.ИдентификаторыОбъектовМетаданных", "Справочник.ИдентификаторыОбъектовРасширений");
	КонецЕсли;
...
Процедура ОбновитьСвойстваИдентификатора(Объект) Экспорт
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		СтарыеЗначения = БазоваяПодсистемаСервер.ЗначенияРеквизитовОбъекта(
			Объект.Ссылка,
			"Наименование,
			|ПорядокКоллекции,
			|Имя,
...
Функция ДанныеОбновлены(Обновить = Ложь, ОбъектыРасширений = Ложь) Экспорт
	Попытка
		Обновлен = РегистрыСведений.ПараметрыРаботыПрограммы.ПараметрРаботыПрограммы("СтандартныеПодсистемы.ИдентификаторыОбъектовМетаданных");
	Исключение
		Если Обновить Тогда
			ВызватьИсключение;
		КонецЕсли;
	...
	Для каждого ПланОбмена Из Метаданные.ПланыОбмена Цикл
		Если ПланОбмена.РаспределеннаяИнформационнаяБаза
		   И ПланОбмена.Состав.Содержит(МетаданныеСправочника)Тогда
			...
			ПланОбменаМенеджер	= БазоваяПодсистемаСервер.МенеджерОбъектаПоПолномуИмени(ПланОбмена.ПолноеИмя());
...
Процедура ФормаСпискаПриСозданииНаСервере(Форма) Экспорт
	...
	Если Параметры.РежимВыбора Тогда
		БазоваяПодсистемаСервер.УстановитьКлючНазначенияФормы(Форма, "ВыборПодбор");
...
Процедура ФормаЭлементаПриСозданииНаСервере(Форма) Экспорт
	...
	Если НЕ ПользователиСервер.ЭтоПолноправныйПользователь(, Не ОбъектыРасширений)
	 ИЛИ ЗапрещеноИзменятьПолноеИмя(Форма.Объект)
	 ИЛИ Не ОбъектыРасширений И БазоваяПодсистемаСервер.ЭтоПодчиненныйУзелРИБ()
...
Процедура ПередЗаписьюОбъекта(Объект) Экспорт
	ОбъектыРасширений = ЭтоОбъектРасширений(Объект);
	БазоваяПодсистемаСерверПовтИсп.ИдентификаторыОбъектовМетаданныхПроверкаИспользования(, ОбъектыРасширений);
	...
	Для Каждого ПланОбмена Из БазоваяПодсистемаСерверПовтИсп.ПланыОбменаРИБ() Цикл
		БазоваяПодсистемаСервер.ЗарегистрироватьОбъектНаВсехУзлах(Объект, ПланОбмена, Ложь);
	КонецЦикла;
...
Функция ОбъектыМетаданныхПоИдентификаторамСПопыткойПовтора(Идентификаторы, КромеНесуществующихИНедоступных) Экспорт
	...
	Для Каждого ТекущийИдентификатор Из Идентификаторы Цикл
		...
		ИначеЕсли ТипЗнч(ТекущийИдентификатор) = Тип("СправочникСсылка.ИдентификаторыОбъектовРасширений") Тогда
			...
			КонецЕсли;
		Иначе
			ВызватьИсключение СтрШаблон("Ошибка при выполнении функции Справочники.ИдентификаторыОбъектовМетаданных.ОбъектыМетаданныхПоИдентификаторамСПопыткойПовтора().
				           |
	...
	Попытка
		ОбъектыМетаданных = ОбъектыМетаданныхПоИдентификаторамБезПопыткиПовтора(Идентификаторы, ИдентификаторыКонфигурации, ИдентификаторыРасширений, КромеНесуществующихИНедоступных);
	Исключение
...
Функция ОбъектыМетаданныхПоИдентификаторамБезПопыткиПовтора(Идентификаторы, ИдентификаторыКонфигурации, ИдентификаторыРасширений, КромеНесуществующихИНедоступных)
	УстановитьПривилегированныйРежим(Истина);
	...
	Выгрузка		= Запрос.Выполнить().Выгрузить();
	ЗаголовокОшибки	= "Ошибка при выполнении функции Справочники.ИдентификаторыОбъектовМетаданных.ОбъектыМетаданныхПоИдентификаторамСПопыткойПовтора()";
	...
	ВерсияПрограммыОбновленаДинамически = БазоваяПодсистемаСервер.ВерсияПрограммыОбновленаДинамически();
...
Функция КлючОбъектаМетаданныхСоответствуетПолномуИмени(СвойстваИдентификатора)
	...
	Иначе
		...
		Если ОбъектМетаданных = Неопределено Тогда
			...
			Строка = БазоваяПодсистемаСерверПовтИсп.СвойстваКоллекцийОбъектовМетаданных(ОбъектыРасширений).Найти(СвойстваИдентификатора.Ссылка.УникальныйИдентификатор(), "Идентификатор");
			Если Строка <> Неопределено Тогда
