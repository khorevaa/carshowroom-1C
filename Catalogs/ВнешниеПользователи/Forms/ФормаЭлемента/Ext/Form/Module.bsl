...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПользователиСерверПовтИсп.ПустыеСсылкиТиповОбъектовАвторизации().Количество() <= 0 Тогда
		Элементы.ОбъектАвторизации.Доступность = Ложь;
	КонецЕсли;

	// Заполнение вспомогательных данных.

	ЗапретРедактированияРолей = (ПользователиСерверПовтИсп.Настройки().РедактированиеРолей <> Истина);
	...
	УровеньДоступа	= ПользователиСервер.сУровеньДоступаКСвойствамПользователя(Объект);
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда
			...
			Если Не ПользователиСервер.сУровеньДоступаПользователяВыше(ЗначениеКопирования, УровеньДоступа) Тогда
		...
		Иначе
			...
			ИначеЕсли ЗначениеЗаполнено(ГруппаНовогоВнешнегоПользователя) Тогда
				НазначениеГруппыВнешнихПользователей = БазоваяПодсистемаСервер.ЗначениеРеквизитаОбъекта(ГруппаНовогоВнешнегоПользователя, "Назначение").Выгрузить();
		...
		Если УровеньДоступа.УправлениеСписком И Объект.ОбъектАвторизации <> Неопределено Тогда
			ПользовательИБИмя		= ПользователиКлиентСервер.ПолучитьКраткоеИмяПользователяИБ(ТекущееПредставлениеОбъектаАвторизации);
	...
	Элементы.ПотребоватьСменуПароляПриВходе.РасширеннаяПодсказка.Заголовок = ПользователиСервер.сПодсказкаПотребоватьСменуПароляПриВходе(Истина);
КонецПроцедуры
...
Процедура ОбработатьИнтерфейсРолейУстановитьТолькоПросмотрРолей(ТекущийОбъект = Неопределено)
	...
	ОбработатьИнтерфейсРолей("УстановитьТолькоПросмотрРолей", ЗапретРедактированияРолей Или ДействияВФорме.Роли <> "Редактирование" Или Не УровеньДоступа.НастройкиДляВхода Или Не ТекущийОбъект.УстановитьРолиНепосредственно);
КонецПроцедуры
...
Процедура ОбщаяНастройкаФормы(ТекущийОбъект, ПриСозданииНаСервере = Ложь, ПараметрыЗаписи = Неопределено)
	...
	УстановитьПривилегированныйРежим(Истина);
	ПользователиСервер.сПрочитатьСведенияОПользователе(ЭтотОбъект);
	УстановитьПривилегированныйРежим(Ложь);

	УровеньДоступа = ПользователиСервер.сУровеньДоступаКСвойствамПользователя(ТекущийОбъект);
	...
	Элементы.УстановитьРолиНепосредственно.Видимость	= ЗначениеЗаполнено(ДействияВФорме.Роли) И ПользователиСерверПовтИсп.Настройки().РедактированиеРолей;

	ОбновитьОтображаемыйТипПользователя();

	ТолькоПросмотр = ТолькоПросмотр ИЛИ ДействияВФорме.Роли <> "Редактирование" И ДействияВФорме.СвойстваЭлемента <> "Редактирование" И ДействияВФорме.СвойстваПользователяИБ <> "Редактирование";
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ДействияВФорме.Роли = "Редактирование" И Объект.УстановитьРолиНепосредственно И ПользовательИБРоли.Количество() = 0 Тогда
		Если НЕ ПараметрыЗаписи.Свойство("СПустымСпискомРолей") Тогда
			Отказ = Истина;
			ПоказатьВопрос(Новый ОписаниеОповещения("ПослеОтветаНаВопросОЗаписиСПустымСпискомРолей", ЭтотОбъект, ПараметрыЗаписи), "Пользователю информационной базы не установлено ни одной роли. Продолжить?", РежимДиалогаВопрос.ДаНет, , , "Запись пользователя информационной базы");
			Возврат;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ДействияВФорме.СвойстваЭлемента <> "Редактирование" Тогда
		ЗаполнитьЗначенияСвойств(ТекущийОбъект, БазоваяПодсистемаСервер.ЗначенияРеквизитовОбъекта(ТекущийОбъект.Ссылка, "ПометкаУдаления"));
	КонецЕсли;
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	УстановитьПривилегированныйРежим(Истина);
	ПользователиСервер.сЗаписатьСведенияОПользователе(ЭтотОбъект, ТекущийОбъект);
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	ПользователиКлиент.сРазвернутьПодсистемыРолей(ЭтотОбъект);
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	ТекстОшибки = "";
	Если ПользователиСервер.сОбъектАвторизацииИспользуется(Объект.ОбъектАвторизации, Объект.Ссылка, , , ТекстОшибки) Тогда
		БазоваяПодсистемаКлиентСервер.СообщитьПользователю(ТекстОшибки, , "Объект.ОбъектАвторизации", , Отказ);
	КонецЕсли;
	...
	Если ТребуетсяЗаписьПользователяИБ(ЭтотОбъект) Тогда
		...
		ПользователиСервер.сПроверитьОписаниеПользователяИБ(ОписаниеПользователяИБ, Отказ);

		ТекстСообщения = "";
		Если ПользователиСервер.сТребуетсяСоздатьПервогоАдминистратора(Неопределено, ТекстСообщения) Тогда
			БазоваяПодсистемаКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ВходВПрограммуРазрешен", , Отказ);
		КонецЕсли;
	КонецЕсли;
		...
		Для Каждого Строка Из ЭлементыДерева Цикл
			...
			Если Строка.ЭтоНедоступнаяРоль Тогда
				БазоваяПодсистемаКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					...
					"Роли",
					ЭлементыДерева.Индекс(Строка),
					СтрШаблон("Роль ""%1"" в строке %%1 недоступна для внешних пользователей.", Строка.Синоним));
			КонецЕсли;
		КонецЦикла;
...
Процедура НедействителенПриИзменении(Элемент)
	Если Объект.Недействителен Тогда
		ВходВПрограммуРазрешен = Ложь;
	Иначе
		ВходВПрограммуРазрешен = ВходВПрограммуРазрешенЗначениеПрямогоИзменения И (ПользовательИБАутентификацияOpenID Или ПользовательИБАутентификацияСтандартная);
	КонецЕсли;
...
Процедура СменитьПароль(Команда)
	...
	ПользователиКлиент.сОткрытьФормуСменыПароля(Объект.Ссылка, Новый ОписаниеОповещения("СменитьПарольПослеПолученияПароля", ЭтотОбъект), ДополнительныеПараметры);
КонецПроцедуры
...
Процедура ВключитьРоли(Команда)
	...
	ПользователиКлиент.сРазвернутьПодсистемыРолей(ЭтотОбъект, Ложь);
КонецПроцедуры
...
Процедура ОбновитьИмяДляВхода(Форма, ПриИзмененииНаименования = Ложь)
	...
	КраткоеИмя = ПользователиКлиентСервер.ПолучитьКраткоеИмяПользователяИБ(Форма.ТекущееПредставлениеОбъектаАвторизации);
...
Функция ОписаниеПользователяИБ()
	...
	Если УровеньДоступа.НастройкиДляВхода И ПользователиСерверПовтИсп.Настройки().РедактированиеРолей И Объект.УстановитьРолиНепосредственно Тогда
...
Процедура ОбновитьОтображаемыйТипПользователя()
	Если БазоваяПодсистемаСервер.ЭтоСсылка(ТипЗнч(Объект.ОбъектАвторизации)) Тогда
...
Процедура ПрочитатьРолиПользователяИБ()
	СвойстваПользователяИБ = ПользователиСервер.СвойстваПользователяИБ(Объект.ИдентификаторПользователяИБ);
	Если СвойстваПользователяИБ = Неопределено Тогда
		СвойстваПользователяИБ = ПользователиСервер.НовоеОписаниеПользователяИБ();
	КонецЕсли;
...
Процедура ПрочитатьПользователяИБ(ПриКопированииЭлемента = Ложь)
	...
	Если ПользовательИБОсновной И Не ВходВПрограммуРазрешен Тогда
		ХранимыеСвойства						= ПользователиСервер.сХранимыеСвойстваПользователяИБ(Объект.Ссылка);
...
Процедура ОпределитьНесоответствияПользователяСПользователемИБ(ПараметрыЗаписи = Неопределено)
	...
	ИначеЕсли УровеньДоступа.УправлениеСписком Тогда
		...
		Если УточнениеСвойств.Количество() > 0 Тогда
			...
			КонецЕсли;
			Если ПоказатьКомандыУстраненияРазличий Или ЕстьРазличияУстранимыеБезАдминистратора И ДействияВФорме.СвойстваЭлемента = "Редактирование" Тогда
...
Процедура УстановитьДоступностьСвойств(Форма)
	...
	Элементы.ОбъектАвторизации.ТолькоПросмотр		= ДействияВФорме.СвойстваЭлемента <> "Редактирование" Или Форма.ОбъектАвторизацииЗаданПриОткрытии Или ЗначениеЗаполнено(Объект.Ссылка) И ЗначениеЗаполнено(Объект.ОбъектАвторизации);
...
Процедура ОбработатьИнтерфейсРолей(Действие, ОсновнойПараметр = Неопределено)
	...
	ПользователиСервер.сОбработатьИнтерфейсРолей(Действие, ПараметрыДействия);
КонецПроцедуры
