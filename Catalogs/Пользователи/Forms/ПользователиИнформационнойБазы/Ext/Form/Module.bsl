...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПользователиСервер.НайтиНеоднозначныхПользователейИБ(Неопределено);
...
Процедура ЗаполнитьПользователейИБ()
	...
	Для Каждого ПользовательИБ Из ВсеПользователиИБ Цикл
		...
		Если СвойстваПользовательИБ = Неопределено Тогда
			СвойстваПользовательИБ = ПользователиСервер.НовоеОписаниеПользователяИБ();
		КонецЕсли;
...
Процедура УдалитьПользователяИБ(ИдентификаторПользователяИБ, Отказ)
	Попытка
		ПользователиСервер.УдалитьПользователяИБ(ИдентификаторПользователяИБ);
	Исключение
		БазоваяПодсистемаКлиентСервер.СообщитьПользователю(КраткоеПредставлениеОшибки(ИнформацияОбОшибке()), , , , Отказ);
	КонецПопытки;
КонецПроцедуры

&НаКлиенте
Процедура УдалитьТекущегоПользователяИБ(УдалитьСтроку = Ложь)
	ПоказатьВопрос(Новый ОписаниеОповещения("УдалитьТекущегоПользователяИБЗавершение", ЭтотОбъект, УдалитьСтроку),
		"Удалить пользователя информационной базы?",
...
Процедура СопоставитьПользователяИБ(СНовым = Ложь)
	Если ТипыПользователей.Количество() > 1 Тогда
		ТипыПользователей.ПоказатьВыборЭлемента(Новый ОписаниеОповещения("СопоставитьПользователяИБДляТипаЭлемента", ЭтотОбъект, СНовым),
			"Выбор типа данных",
			ТипыПользователей[0]);
	Иначе
