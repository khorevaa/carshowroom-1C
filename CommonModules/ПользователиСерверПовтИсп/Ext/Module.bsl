...
Функция Настройки() Экспорт
	...
	Возврат БазоваяПодсистемаСервер.ФиксированныеДанные(ВсеНастройки);
КонецФункции
...
Функция ДеревоРолей(ПоПодсистемам = Истина, Назначение = "ДляПользователей") Экспорт
	ПроверитьНазначение(Назначение,  "Ошибка в функции ДеревоРолей общего модуля ПользователиСерверПовтИсп.");

	НедоступныеРоли = ПользователиСерверПовтИсп.НедоступныеРоли(Назначение);
...
Функция ПустыеСсылкиТиповОбъектовАвторизации() Экспорт
	...
	Для Каждого Тип Из Метаданные.ОпределяемыеТипы.ВнешнийПользователь.Тип.Типы() Цикл
		Если Не БазоваяПодсистемаСервер.ЭтоСсылка(Тип) Тогда
			Продолжить;
		КонецЕсли;
