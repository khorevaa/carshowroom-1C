...
Процедура УстановкаПараметровСеанса(Знач ИмяПараметра, УстановленныеПараметры) Экспорт
	...
	ИначеЕсли ИмяПараметра = "ОтмененныеДлительныеОперации" Тогда
...
//
Процедура ЗаписатьОбъект(Знач Объект, Знач РегистрироватьНаУзлахПлановОбмена = Неопределено, Знач ВключитьБизнесЛогику = Ложь, ДокументРежимЗаписи = Неопределено) Экспорт
...
Функция ЗаполнитьДанныеОтложенногоОбработчика(ОписаниеПроцедуры, ПараметрыОтметкиКОбработке)
	Если ПараметрыОтметкиКОбработке = Неопределено Тогда
		ПараметрыОтметкиКОбработке = ОсновныеПараметрыОтметкиКОбработке();

		// Зарезервирповано для новых подсистем
	КонецЕсли;
...
Функция ПоказатьОписаниеИзмененийСистемы() Экспорт
	...
	Если Не ПравоДоступа("Просмотр", Метаданные.ОбщиеФормы.ОписаниеИзмененийСистемы) Тогда
		Возврат Ложь;
	КонецЕсли;

	ВывестиОписаниеИзмененийДляАдминистратора = БазоваяПодсистемаСервер.ХранилищеОбщихНастроекЗагрузить("ОбновлениеИБ", "ВывестиОписаниеИзмененийДляАдминистратора",,, ИмяПользователя());
	...
	Если МакетОписаниеОбновлений <> Неопределено Тогда
		...
		Для каждого Область Из Макет.Области Цикл
			...
			Если ВесВерсии > ВесВерсииМетаданных Тогда
				ТекстИсключения = "В общем макете ОписаниеИзмененийСистемы для одного из разделов изменений
...
Функция ОбъектОбработан(Данные) Экспорт
	...
	Для Каждого Обработчик Из ОбработчикиОбъекта Цикл
		...
		ИначеЕсли ТипЗнч(Данные) = Тип("Строка") Тогда
			Обработан = Ложь;
		Иначе
			Обработан = БазоваяПодсистемаСервер.ВычислитьВБезопасномРежиме(СвойстваОбработчика.ПроцедураПроверки + "(Параметры)", МетаданныеИОтбор);
		КонецЕсли;
...
Функция МетаданныеИОтборПоДанным(Данные, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ИначеЕсли ТипДанных = Тип("ДанныеФормыСтруктура") Тогда
		Если БазоваяПодсистемаКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Данные, "Ссылка") Тогда
			...
			Если Не ЗначениеЗаполнено(Отбор) Тогда
				ЭтоНовый = Истина;
			КонецЕсли;
		ИначеЕсли БазоваяПодсистемаКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Данные, "ИсходныйКлючЗаписи") Тогда
			Если МетаданныеОбъекта = Неопределено Тогда
			...
			КонецЦикла;
		Иначе
			ТекстИсключения = "Процедура ОбновлениеИБСервер.МетаданныеИОтборПоДанным не может быть использована для этой формы.";
		КонецЕсли;
	Иначе
		Если МетаданныеОбъекта = Неопределено Тогда
		...
		Если БазоваяПодсистемаСервер.ЭтоОбъектСсылочногоТипа(МетаданныеОбъекта) Тогда
			Если БазоваяПодсистемаСервер.ЭтоСсылка(ТипДанных) Тогда
				Отбор = Данные;
			Иначе
				Отбор = Данные.Ссылка;
		...
		ИначеЕсли БазоваяПодсистемаСервер.ЭтоРегистр(МетаданныеОбъекта) Тогда
			...
			КонецЕсли;
		Иначе
			ТекстИсключения = "Для этого типа метаданных не поддерживается анализ в функции ОбновлениеИБСервер.МетаданныеИОтборПоДанным.";
