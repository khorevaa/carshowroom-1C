...
Процедура ЗаполнитьНазначениеГруппВнешнихПользователей() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ГруппаОбъект.Предопределенный Тогда // любые пользователи
			ПустыеСсылки = ПользователиСерверПовтИсп.ПустыеСсылкиТиповОбъектовАвторизации();
...
Процедура НайтиНеоднозначныхПользователейИБ(Знач Пользователь, Знач УникальныйИдентификатор = Неопределено, Знач НайденныеИдентификаторы = Неопределено) Экспорт
	...
	Если ТипЗнч(УникальныйИдентификатор) <> Тип("УникальныйИдентификатор") Или УникальныйИдентификатор = ПустойУникальныйИдентификатор Тогда
	...
	Иначе
		...
		Если ТипЗнч(Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "Справочник.Пользователи КАК СправочникПользователи", "Справочник.ВнешниеПользователи КАК СправочникПользователи");
		КонецЕсли;
	КонецЕсли;
	...
	Для каждого Строка Из Выгрузка Цикл
		Если Строка.НеоднозначныйИдентификатор <> ТекущийНеоднозначныйИдентификатор Тогда
			...
			Иначе
				...
				ОписаниеОшибки = ОписаниеОшибки + СтрШаблон("Пользователю ИБ ""%1"" с идентификатором ""%2"" соответствует более одного элемента в справочнике:", ИмяДляВхода, ТекущийНеоднозначныйИдентификатор);
				ОписаниеОшибки = ОписаниеОшибки + Символы.ПС;
			КонецЕсли;
		КонецЕсли;
		...
		Иначе
			ОписаниеОшибки = ОписаниеОшибки + "- " + СтрШаблон("""%1"" %2", Строка.Пользователь, ПолучитьНавигационнуюСсылку(Строка.Пользователь)) + Символы.ПС
		КонецЕсли;
	КонецЦикла;
...
Процедура СкопироватьСвойстваПользователяИБ(Приемник, Источник, КопируемыеСвойства = "", ИсключаемыеСвойства = "", ПрефиксСвойств = "") Экспорт
	Если ТипЗнч(Приемник) = Тип("ПользовательИнформационнойБазы") И ТипЗнч(Источник) = Тип("ПользовательИнформационнойБазы")
		Или ТипЗнч(Приемник) = Тип("ПользовательИнформационнойБазы") И ТипЗнч(Источник) <> Тип("Структура") И ТипЗнч(Источник) <> Тип("УправляемаяФорма")
		Или ТипЗнч(Источник) = Тип("ПользовательИнформационнойБазы") И ТипЗнч(Приемник) <> Тип("Структура") И ТипЗнч(Приемник) <> Тип("УправляемаяФорма") Тогда

		ВызватьИсключение "Недопустимое значение параметра Приемник или Источник
			           |в процедуре СкопироватьСвойстваПользователяИБ общего модуля ПользователиСервер.";
	КонецЕсли;
	...
	Для каждого КлючИЗначение Из ВсеСвойства Цикл
		...
		Если ТипЗнч(Источник) = Тип("ПользовательИнформационнойБазы") И (ТипЗнч(Приемник) = Тип("Структура") ИЛИ ТипЗнч(Приемник) = Тип("УправляемаяФорма") ) Тогда
...
Процедура ОбновитьДеревоРолей(Параметры)
	...
	ДеревоРолей = ПользователиСерверПовтИсп.ДеревоРолей(ПоказатьПодсистемыРолей, НазначениеРолей).Получить();
	...
	ИначеЕсли Действие = "ЗаполнитьРоли" Тогда
		...
		Иначе
			...
			Для Каждого Строка Из ПрочитанныеРоли Цикл
				...
				КонецЕсли;
			КонецЦикла;
			ПрочитанныеРоли = Справочники.ИдентификаторыОбъектовМетаданных.ОбъектыМетаданныхПоИдентификаторамСПопыткойПовтора(ИдентификаторыРолей, Истина);
...
Процедура сОбновитьРолиВнешнихПользователей(Знач МассивВнешнихПользователей = Неопределено) Экспорт
	...
	Попытка
		...
		Запрос.УстановитьПараметр("ВнешниеПользователи", МассивВнешнихПользователей);
		Запрос.УстановитьПараметр("ВсеРоли", ПользователиСерверПовтИсп.ВсеРоли().Таблица.Получить());
...
Процедура сЗавершитьОбработкуПользователяИБ(ПользовательОбъект, ПараметрыОбработки) Экспорт
	...
	Если ПараметрыОбработки.Действие = "Записать" И ВходВПрограммуРазрешен(ПараметрыОбработки.ОписаниеНовогоПользователяИБ) Тогда
		УстановитьПривилегированныйРежим(Истина);
		сОбновитьСведенияОПользователеСРазрешеннымВходом(ПользовательОбъект.Ссылка, Не ПараметрыОбработки.СтарыйПользовательИБСуществует Или Не ПользователиСервер.ВходВПрограммуРазрешен(ПараметрыОбработки.ОписаниеСтарогоПользователяИБ));
		УстановитьПривилегированныйРежим(Ложь);
	КонецЕсли;
...
Процедура сОбновитьСведенияОПользователеСРазрешеннымВходом(Пользователь, ВходВПрограммуВключен)
	Блокировка			= Новый БлокировкаДанных;
	ЭлементБлокировки	= Блокировка.Добавить("РегистрСведений.СведенияОПользователях");
	ЭлементБлокировки.УстановитьЗначение("Пользователь", Пользователь);
...
Функция сПодсказкаПотребоватьСменуПароляПриВходе(ДляВнешнихПользователей) Экспорт
	ЭтоПолноправныйПользователь = ЭтоПолноправныйПользователь(, Ложь);
	...
	Если Не ЭтоПолноправныйПользователь Тогда
		Подсказка.Добавить("За подробностями обратитесь к администратору.");
	...
	Если Не ПользователиСерверПовтИсп.Настройки().ОбщиеНастройкиВхода Тогда
		Подсказка.Добавить("Параметры информационной базы в конфигураторе в меню Администрирование.");
		Возврат Новый ФорматированнаяСтрока(Подсказка);
	КонецЕсли;
