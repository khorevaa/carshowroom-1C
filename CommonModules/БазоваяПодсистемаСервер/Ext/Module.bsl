...
Процедура ПометитьЗаписиКэшаВерсийНеактуальными() Экспорт
	...
	Попытка
		...
		ОбновлениеИБСервер.ЗаписатьДанные(НаборЗаписей);
...
Процедура ПеренестиПаролиВБезопасноеХранилище() Экспорт
	...
	Результат.Логин		= ХранилищеОбщихНастроекЗагрузить("АутентификацияНаПользовательскомСайте", "КодПользователя", "");
	Результат.Пароль	= ХранилищеОбщихНастроекЗагрузить("АутентификацияНаПользовательскомСайте", "Пароль", "");
	Если НЕ ПустаяСтрока(Результат.Логин) Тогда
		Владелец = Справочники.ИдентификаторыОбъектовМетаданных.ИдентификаторОбъектаМетаданных("Справочник.ИдентификаторыОбъектовМетаданных");
		УстановитьПривилегированныйРежим(Истина);
		ЗаписатьДанныеВБезопасноеХранилище(Владелец, Результат.Логин, "Логин");
		ЗаписатьДанныеВБезопасноеХранилище(Владелец, Результат.Пароль);
...
Процедура ПослеОтправкиДанных(Источник, Отказ) Экспорт
	...
	Если Источник.НомерОтправленного = ЗначениеРеквизитаОбъекта(Источник.Ссылка, "НомерОтправленного") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПослеПолученияДанных(Источник, Отказ) Экспорт
	...
	Если Источник.ЭтоНовый() Или Источник.НомерПринятого = ЗначениеРеквизитаОбъекта(Источник.Ссылка, "НомерПринятого") Тогда
		Возврат;
	КонецЕсли;

	ПолучениеИзГлавногоУзла = (ПланыОбмена.ГлавныйУзел() = Источник.Ссылка);

	ИнтеграцияПодсистемСервер.ПослеПолученияДанных(Источник, Отказ, ПолучениеИзГлавногоУзла);
КонецПроцедуры
...
Процедура ЗапретитьПометкуУдаленияПредопределенныхЭлементовПередЗаписью(Источник, Отказ) Экспорт
	Если Источник.ОбменДанными.Загрузка Или Источник.ИмяПредопределенныхДанных = "" Или Источник.ПометкаУдаления <> Истина Тогда
		Возврат;
	КонецЕсли;

	Если Источник.ЭтоНовый() Тогда
		ВызватьИсключение "Недопустимо создавать предопределенный элемент помеченный на удаление.";
	Иначе
		СтарыеСвойства = ЗначенияРеквизитовОбъекта(Источник.Ссылка, "ПометкаУдаления, ИмяПредопределенныхДанных");

		Если СтарыеСвойства.ИмяПредопределенныхДанных <> "" И СтарыеСвойства.ПометкаУдаления <> Истина Тогда
			ВызватьИсключение СтрШаблон("Недопустимо помечать на удаление предопределенный элемент:
				           |""%1"".",
				Строка(Источник.Ссылка));
		ИначеЕсли СтарыеСвойства.ИмяПредопределенныхДанных = "" И СтарыеСвойства.ПометкаУдаления = Истина Тогда
			ВызватьИсключение СтрШаблон("Недопустимо связывать с именем предопределенного элемент, помеченный на удаление:
				           |""%1"".",
				Строка(Источник.Ссылка));
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗапретитьУдалениеПредопределенныхЭлементовПередУдалением(Источник, Отказ) Экспорт
	Если Источник.ОбменДанными.Загрузка Или Источник.ИмяПредопределенныхДанных = "" Тогда
		Возврат;
	КонецЕсли;

	ВызватьИсключение СтрШаблон("Недопустимо удалять предопределенный элемент
		           |""%1"".",
		Строка(Источник.Ссылка));
КонецПроцедуры
...
Процедура ПриОтправкеДанныхГлавномуСобытие(Источник, ЭлементДанных, ОтправкаЭлемента) Экспорт
	...
	ИнтеграцияПодсистемСервер.ПриОтправкеДанныхГлавному(ЭлементДанных, ОтправкаЭлемента, Источник);
...
Процедура ПриОтправкеДанныхПодчиненномуСобытие(Источник, ЭлементДанных, ОтправкаЭлемента, СозданиеНачальногоОбраза) Экспорт
	...
	ИгнорироватьОтправкуДанныхОбработанныхВЦентральномУзлеРИБПриОбновленииИБ(ЭлементДанных, СозданиеНачальногоОбраза, Источник);
	...
	ИнтеграцияПодсистемСервер.ПриОтправкеДанныхПодчиненному(ЭлементДанных, ОтправкаЭлемента, СозданиеНачальногоОбраза, Источник);
	Если ОтправкаЭлемента = ОтправкаЭлементаДанных.Игнорировать Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПриПолученииДанныхОтГлавногоСобытие(Источник, ЭлементДанных, ПолучениеЭлемента, ОтправкаНазад) Экспорт
	...
	ИнтеграцияПодсистемСервер.ПриПолученииДанныхОтГлавного(ЭлементДанных, ПолучениеЭлемента, ОтправкаНазад, Источник);
	Если ПолучениеЭлемента = ПолучениеЭлементаДанных.Игнорировать Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПриПолученииДанныхОтПодчиненногоСобытие(Источник, ЭлементДанных, ПолучениеЭлемента, ОтправкаНазад) Экспорт
	...
	ИнтеграцияПодсистемСервер.ПриПолученииДанныхОтПодчиненного(ЭлементДанных, ПолучениеЭлемента, ОтправкаНазад, Источник);
	Если ПолучениеЭлемента = ПолучениеЭлементаДанных.Игнорировать Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьВозможностьЗапуска()
	...
	Исключение
		ВызватьИсключение СтрШаблон("Невозможно установить параметры сеанса по причине: %1.
					|
...
Функция ТребуетсяПоказРекомендацииПоОбъемуОперативнойПамяти()
	Если БазоваяПодсистемаКлиентСервер.ЭтоВебКлиент() Или Не ИнформационнаяБазаФайловая() Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	ОперативнаяПамять		= ПараметрыКлиентаНаСервере().Получить("ОперативнаяПамять");
	СохраненнаяРекомендация	= ХранилищеОбщихНастроекЗагрузить("ОбщиеНастройкиПользователя", "РекомендацияПоОбъемуОперативнойПамяти");
	...
	Возврат ОперативнаяПамять < РекомендуемыйОбъем И (Рекомендация.Показывать Или (ТекущаяДатаСеанса() - Рекомендация.ДатаПредыдущегоПоказа) > 60*60*24*60)
КонецФункции
...
Процедура ЗарегистрироватьИзменениеПриоритетныхДанныхДляПодчиненныхУзловРИБ() Экспорт
	...
	Для Каждого ПланОбмена Из Метаданные.ПланыОбмена Цикл
		...
		ПланОбменаМенеджер	= МенеджерОбъектаПоПолномуИмени(ПланОбмена.ПолноеИмя());
...
Процедура ИгнорироватьОтправкуИдентификаторовОбъектовМетаданных(ЭлементДанных, ОтправкаЭлемента, Знач СозданиеНачальногоОбраза = Ложь)
	Если Не СозданиеНачальногоОбраза И ОбъектМетаданных(ЭлементДанных) = Метаданные.Справочники.ИдентификаторыОбъектовМетаданных Тогда
...
Процедура ИгнорироватьОтправкуДанныхОбработанныхВЦентральномУзлеРИБПриОбновленииИБ(ЭлементДанных, СозданиеНачальногоОбраза, Получатель)
	...
	Если Получатель <> Неопределено И Не СозданиеНачальногоОбраза И ТипЗнч(ЭлементДанных) = Тип("РегистрСведенийНаборЗаписей.ДанныеОбработанныеВЦентральномУзлеРИБ") Тогда
...
Процедура УстановитьПустуюФормуНаРабочийСтол() Экспорт
	...
	Если ТипЗнч(ТекущиеНастройки) = Тип("НастройкиНачальнойСтраницы") Тогда
		...
		Если ТекущийСоставФорм.ПраваяКолонка.Количество() = 0 И ТекущийСоставФорм.ЛеваяКолонка.Количество() = 1 И ТекущийСоставФорм.ЛеваяКолонка[0] = "ОбщаяФорма.ПустойРабочийСтол" Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Функция ЗначениеРеквизитаОбъекта(Ссылка, ИмяРеквизита, ВыбратьРазрешенные = Ложь) Экспорт
	Если ПустаяСтрока(ИмяРеквизита) Тогда
		ВызватьИсключение "Неверный второй параметр ИмяРеквизита:
...
Функция ЗначенияРеквизитовОбъекта(Ссылка, Знач Реквизиты, ВыбратьРазрешенные = Ложь) Экспорт
	...
	Если ТипЗнч(Ссылка) = Тип("Строка") Тогда
		...
		Попытка
			Ссылка = БазоваяПодсистемаКлиентСервер.ПредопределенныйЭлемент(ПолноеИмяПредопределенногоЭлемента);
		Исключение
			ТекстОшибки = СтрШаблон("Неверный первый параметр Ссылка: %1", КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Иначе // Если передана ссылка.
		...
		Исключение
			ВызватьИсключение "Неверный первый параметр Ссылка:
		...
		Для Каждого Реквизит Из Реквизиты Цикл
			...
			Исключение
				...
				Если Результат.Ошибка Тогда
					ВызватьИсключение СтрШаблон("Неверный второй параметр Реквизиты: %1", Результат.ОписаниеОшибки);
				КонецЕсли;
...
Функция НайтиОшибкуДоступностиРеквизитовОбъекта(ПолноеИмяОбъектаМетаданных, ПроверяемыеВыражения)
	...
	Для Каждого ТекущееВыражение Из ПроверяемыеВыражения Цикл
		Если Не ПолеИсточникаСхемыЗапросаДоступно(Источник, ТекущееВыражение) Тогда
			ТекстОшибки = ТекстОшибки + Символы.ПС + СтрШаблон("- Поле объекта ""%1"" не найдено", ТекущееВыражение);
		КонецЕсли;
	КонецЦикла;
...
Функция ОбъектПоПолномуИмени(ПолноеИмя) Экспорт
	...
	Если МассивСтрок.Количество() >= 2 Тогда
		...
		Имя = МассивСтрок[1];
	Иначе
		ВызватьИсключение СтрШаблон("Некорректное полное имя отчета или обработки ""%1"".", ПолноеИмя);
	КонецЕсли;
	...
	Иначе
		ВызватьИсключение СтрШаблон("""%1"" не является отчетом или обработкой.", ПолноеИмя);
	КонецЕсли;
КонецФункции
...
Функция ДанныеСовпадают(Данные1, Данные2) Экспорт
	...
	Если ТипЗнч(Данные1) = Тип("Структура") ИЛИ ТипЗнч(Данные1) = Тип("ФиксированнаяСтруктура") Тогда
		...
		Для каждого КлючИЗначение Из Данные1 Цикл
			...
			Если НЕ Данные2.Свойство(КлючИЗначение.Ключ, СтароеЗначение) ИЛИ НЕ ДанныеСовпадают(КлючИЗначение.Значение, СтароеЗначение) Тогда
				Возврат Ложь;
			КонецЕсли;
		КонецЦикла;
...
Функция ОбъектМетаданныхДоступенПоФункциональнымОпциям(Знач ОбъектМетаданных) Экспорт
	...
	Возврат БазоваяПодсистемаСерверПовтИсп.ДоступностьОбъектовПоОпциям().Получить(ПолноеИмя) <> Ложь;
КонецФункции
...
#Если ВнешнееСоединение Тогда
	...
	Если ПараметрыВыполнения.БезРасширений И ИнформационнаяБазаФайловая Тогда
		ВызватьИсключение "Фоновое задание не может быть запущено с параметром ""БезРасширений"" в файловой информационной базе в БазоваяПодсистемаСервер.ВыполнитьВФоне.'");
	КонецЕсли;
#КонецЕсли
...
#Если ВнешнееСоединение Тогда
	ВыполнитьБезФоновогоЗадания = ИнформационнаяБазаФайловая
		Или БазоваяПодсистемаКлиентСервер.РежимОтладки() Или ПараметрыВыполнения.ЗапуститьНеВФоне
...
Процедура ИзменитьЗадание(Знач Идентификатор, Знач Параметры) Экспорт
	...
	ПараметрыЗадания = БазоваяПодсистемаКлиентСервер.СкопироватьСтруктуру(Параметры);
