...
Процедура ОтметитьВыполнениеОбработки(Очередь, Данные, ДополнительныеПараметры) Экспорт
	Если (ПараметрыСеанса.ПараметрыОбработчикаОбновления.ЗапускатьТолькоВГлавномУзле
			И Не БазоваяПодсистемаСерверПовтИсп.ИспользуетсяРИБ())
			Или (Не ПараметрыСеанса.ПараметрыОбработчикаОбновления.ЗапускатьТолькоВГлавномУзле И Не БазоваяПодсистемаСерверПовтИсп.ИспользуетсяРИБ("СФильтром")) Тогда
		Возврат;
	КонецЕсли;
	...
	Если ПараметрыСеанса.ПараметрыОбработчикаОбновления.ЗапускатьТолькоВГлавномУзле Тогда
		УзлыРИБ = БазоваяПодсистемаСерверПовтИсп.УзлыРИБ();
	Иначе
		УзлыРИБ = БазоваяПодсистемаСерверПовтИсп.УзлыРИБ("СФильтром");
	КонецЕсли;
	...
	Если ДополнительныеПараметры.ЭтоДвижения Или ДополнительныеПараметры.ЭтоНезависимыйРегистрСведений Тогда
		ИдентификаторОбъектаМетаданных = БазоваяПодсистемаСервер.ИдентификаторОбъектаМетаданных(ДополнительныеПараметры.ПолноеИмяРегистра);
	Иначе
		Если ТипДанных = Тип("Массив") Тогда
			ИдентификаторОбъектаМетаданных = Неопределено;
		Иначе
			ИдентификаторОбъектаМетаданных = БазоваяПодсистемаСервер.ИдентификаторОбъектаМетаданных(ТипДанных);
		КонецЕсли;
	КонецЕсли;
	...
	Иначе
		Если ТипЗнч(Данные) <> Тип("Массив") Тогда
			...
			Если Метаданные.РегистрыСведений.Содержит(ОбъектМетаданных) И ОбъектМетаданных.РежимЗаписи = Метаданные.СвойстваОбъектов.РежимЗаписиРегистра.Независимый Тогда
				СтруктураЗначенийИзмерений = Новый Структура;
				ГУИД                       = Новый УникальныйИдентификатор;
...
Процедура ДобавитьДанныеВНабор(УзлыРИБ, НаборДанных, Данные, ОбъектМетаданных = Неопределено, Очередь = Неопределено)
	Если ТипЗнч(Данные) = Тип("Массив") Тогда
		Для Каждого СтрокаМассива Из Данные Цикл
			...
			НаборДанных.Отбор.Очередь.Установить(Очередь);
			НаборДанных.Отбор.ОбъектМетаданных.Установить(БазоваяПодсистемаСервер.ИдентификаторОбъектаМетаданных(ТипЗнч(СтрокаМассива)));
	...
	Иначе
		...
		Если БазоваяПодсистемаСервер.ЭтоСсылка(ТипЗнч(Данные)) Тогда
			Значение = Данные;
		ИначеЕсли БазоваяПодсистемаСервер.ЭтоОбъектСсылочногоТипа(ОбъектМетаданных) Тогда
		...
		Если Не НаборДанных.Отбор.ОбъектМетаданных.Использование Или Не ЗначениеЗаполнено(НаборДанных.Отбор.ОбъектМетаданных.Значение) Тогда
			НаборДанных.Отбор.ОбъектМетаданных.Установить(БазоваяПодсистемаСервер.ИдентификаторОбъектаМетаданных(ТипЗнч(Значение)));
		КонецЕсли;
