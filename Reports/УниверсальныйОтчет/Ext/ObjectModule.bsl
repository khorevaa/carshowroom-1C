...
Процедура ПередЗагрузкойВариантаНаСервере(Форма, НовыеНастройкиКД) Экспорт
	...
	Если ТипЗнч(НовыеНастройкиКД) = Тип("НастройкиКомпоновкиДанных") Или НовыеНастройкиКД = Неопределено Тогда
		...
		КонецЕсли;
		ЗагруженнаяСхема = БазоваяПодсистемаКлиентСервер.СвойствоСтруктуры(ДопСвойстваНастроек, "СхемаКомпоновкиДанных");
	...
	Если ЭтоЗагруженнаяСхема Тогда
		...
		ВариантыОтчетовСервер.ПодключитьСхему(ЭтотОбъект, Форма, НоваяСхема, КлючСхемы);
	КонецЕсли;
КонецПроцедуры
...
//   Чтобы изменения схемы вступили в силу следует вызывать метод ВариантыОтчетовСервер.ПодключитьСхему().
//
// Параметры:
...
//		ВариантыОтчетовСервер.ПодключитьСхему(ЭтотОбъект, Контекст, СхемаКД, КлючСхемы);
//	КонецЕсли;
//
...
Процедура ПередЗагрузкойНастроекВКомпоновщик(Контекст, КлючСхемы, КлючВарианта, НовыеНастройкиКД, НовыеПользовательскиеНастройкиКД) Экспорт
	...
	Если НовыйКлючСхемы = Неопределено Тогда// Не загруженная схема.
		...
		НовыйКлючСхемы = ПараметрыОтчета.ТипОбъектаМетаданных + "/" + ПараметрыОтчета.ПолноеИмяОбъектаМетаданных + "/" + ПараметрыОтчета.ИмяТаблицы;
		НовыйКлючСхемы = БазоваяПодсистемаСервер.СократитьСтрокуКонтрольнойСуммой(НовыйКлючСхемы, 100);
	...
	Если НовыйКлючСхемы <> Неопределено И КлючСхемы <> НовыйКлючСхемы Тогда
		КлючСхемы = НовыйКлючСхемы;
		ВариантыОтчетовСервер.ПодключитьСхему(ЭтотОбъект, Контекст, НоваяСхема, КлючСхемы);
		Если ЭтоЗагруженнаяСхема Тогда
