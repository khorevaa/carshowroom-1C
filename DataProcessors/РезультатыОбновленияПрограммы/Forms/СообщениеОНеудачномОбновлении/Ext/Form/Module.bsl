...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаписьЖурналаРегистрации("Обновление информационной базы", УровеньЖурналаРегистрации.Ошибка, , , Параметры.ПодробноеПредставлениеОшибки);
...
#КонецЕсли
	...
	ДиалогОткрытияФайла.Фильтр				= "Внешняя обработка (*.epf)|*.epf";
...
Функция ПодключитьВнешнююОбработку(АдресВоВременномХранилище)
	...
	Если БазоваяПодсистемаСерверПовтИсп.ЕстьЗащитаОтОпасныхДействий() Тогда
		ИмяОбработки = Менеджер.Подключить(АдресВоВременномХранилище, , Ложь, БазоваяПодсистемаСервер.ОписаниеЗащитыБезПредупреждений());
	Иначе
