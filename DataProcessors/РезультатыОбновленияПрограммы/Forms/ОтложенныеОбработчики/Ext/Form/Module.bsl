...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПолноправныйПользователь			= ПользователиСервер.ЭтоПолноправныйПользователь(, Истина);
	...
	Если Не БазоваяПодсистемаКлиентСервер.РежимОтладки() Или СведенияОбОбновлении.ОтложенноеОбновлениеЗавершеноУспешно = Истина Тогда
...
Процедура ДобавитьОтложенныйОбработчик(СтрокаОбработчика, ОбработчикиНеВыполнялись, ВыполненыВсеОбработчики, НачальноеЗаполнение, ВыполняетсяИзменениеПриоритета)
	...
	ШаблонИнтервалВыполнения				= "%1 - %2";
	ШаблонИнформацияОПроцедуреОбновления	= "%1%2Процедура ""%3"" обработки данных %4.";
	...
	Если СтрокаОбработчика.Статус <> "Ошибка" Тогда
		СтрокаСписка.ИнформацияОПроцедуреОбновления = СтрШаблон(ШаблонИнформацияОПроцедуреОбновления, СтрокаОбработчика.Комментарий, Отступ, СтрокаОбработчика.ИмяОбработчика, ПредставлениеСтатусаВыполнения);
	КонецЕсли;
...
Функция ДлительностьВыполненияПроцедурыОбновления(ДлительностьВыполнения)
	...
	ИначеЕсли ДлительностьВСекундах < 60 Тогда
		ШаблонСекунды	= СтрокаСЧислом(";%1 секунда;;%1 секунды;%1 секунд;%1 секунды", Цел(ДлительностьВСекундах),ВидЧисловогоЗначения.Количественное, "L=ru");

		Возврат ШаблонСекунды;
	ИначеЕсли ДлительностьВСекундах < 3600 Тогда
		Минуты			= ДлительностьВСекундах / 60;
		Секунды			= (Минуты - Цел(Минуты)) * 60;
		ШаблонМинуты	= СтрокаСЧислом(";%1 минута;;%1 минуты;%1 минут;%1 минуты", Цел(Минуты),ВидЧисловогоЗначения.Количественное, "L=ru");
		ШаблонМинуты	= ШаблонМинуты + СтрокаСЧислом(";%1 секунда;;%1 секунды;%1 секунд;%1 секунды", Цел(Секунды),ВидЧисловогоЗначения.Количественное, "L=ru");

		Возврат ШаблонМинуты;
	Иначе
		Часы		= ДлительностьВСекундах / 60 / 60;
		Минуты		= (Часы - Цел(Часы)) * 60;
		ШаблонЧасы	= СтрокаСЧислом(";%1 час;;%1 часа;%1 часов;%1 часа", Цел(Часы),ВидЧисловогоЗначения.Количественное, "L=ru");
		ШаблонЧасы	= ШаблонЧасы + СтрокаСЧислом(";%1 минута;;%1 минуты;%1 минут;%1 минуты", Цел(Минуты),ВидЧисловогоЗначения.Количественное, "L=ru");
...
Процедура ОбновитьСтатусыКомандПриоритета(Элемент)
	...
	Если Элемент.ТекущиеДанные.Приоритет = "Неопределено" Или Элемент.ТекущиеДанные.Статус = "Выполнено" Тогда
...
Функция ИзменяемыеОбработчики(Обработчик, ОбрабатываемыеДанныеТаблица, Очередь, ПриоритетЖелательноБыстрее, СписокОбъектов = Неопределено)
	...
	Запрос.Текст	= "ВЫБРАТЬ
	            	  |	втОбрабатываемыеОбъекты.Обработчик КАК Обработчик,
	            	  |	втОбрабатываемыеОбъекты.ИмяОбъекта КАК ИмяОбъекта,
	            	  |	втОбрабатываемыеОбъекты.Очередь КАК Очередь
	            	  |ПОМЕСТИТЬ Таблица
	            	  |ИЗ
	            	  ...
	            	  |////////////////////////////////////////////////////////////////////////////////
	            	  |ВЫБРАТЬ
	            	  |	Таблица.Обработчик КАК Обработчик,
